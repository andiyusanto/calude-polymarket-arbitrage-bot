...
class RiskManager:
    def __init__(self):
        ...
        self.correlated_assets = {"BTC": 0.0, "ETH": 0.0}

    def can_trade(self, daily_pnl: float, portfolio: float) -> bool:
        if portfolio <= 0:
            return False
        loss_pct = (daily_pnl / portfolio) * 100 if daily_pnl < 0 else 0
        if loss_pct <= -CONFIG.max_daily_loss_pct:
            if not self.daily_loss_limit_hit:
                self.daily_loss_limit_hit = True
                log.critical(f"🚨 MAX DAILY LOSS HIT (-{loss_pct:.1f}%) — Trading stopped")
            return False
        return True

    def update_exposure(self, asset: str, size: float, portfolio: float):
        self.correlated_assets[asset] = size / portfolio * 100
